...
export function meta({}: Route.MetaArgs) {
  return [
    { title: "ResumeIQ — Free AI Resume Analyzer & ATS Resume Checker" },
    {
      name: "description",
      content:
        "Upload your resume for an instant AI-powered ATS compatibility score, keyword optimization tips, and personalized feedback to boost your interview chances.",
    },
    {
      tagName: "link",
      rel: "canonical",
    ...
    {
      property: "og:title",
      content: "ResumeIQ — AI Resume Analyzer & ATS Checker",
    },
    {
      property: "og:description",
      content: "Instant AI resume analysis and ATS compatibility score. Free.",
    },
    { property: "og:url", content: "https://ai-parseresume.vercel.app/" },
    {
      property: "og:image",
      content: "https://ai-parseresume.vercel.app/images/resume_01.png",
    },
...
export default function Home() {
  ...
  const bestScore = scoredResumes.length
    ? Math.max(
        ...scoredResumes.map((resume) => resume.feedback?.overallScore || 0),
      )
    : 0;
  ...
  const sortedResumes = [...scoredResumes].sort(
    (first, second) =>
      (second.feedback?.overallScore || 0) -
      (first.feedback?.overallScore || 0),
  );
  ...
  const atsRiskResume = [...scoredResumes].sort(
    (first, second) =>
      (first.feedback?.ATS.score || 0) - (second.feedback?.ATS.score || 0),
  )[0];
  ...
  const statusCounts = applicationStatuses.map((status) => ({
    status,
    count: resumes.filter((resume) => (resume.status || "saved") === status)
      .length,
  }));
      ...
      <div className="relative z-10">
        ...
        <section className="mx-auto flex w-full max-w-[1500px] flex-col gap-8 px-4 pb-12 pt-8 sm:px-8 lg:px-12">
          <div className="grid gap-6 xl:grid-cols-[76px_1fr_380px]">
            ...
            <div
              id="top"
              className="overflow-hidden rounded-3xl border border-white/80 bg-white/85 p-6 shadow-[0_24px_90px_rgba(15,23,42,0.10)] backdrop-blur-xl sm:p-8 lg:p-10"
            >
              ...
              <div className="max-w-4xl">
                <h1 className="text-4xl font-black leading-tight text-slate-950 sm:text-5xl lg:text-6xl">
                  AI Resume Analyzer & ATS Resume Checker
                </h1>

                <p className="mt-5 max-w-3xl text-base leading-7 text-slate-600 sm:text-lg">
                  Upload your resume and get an instant AI-powered resume
                  analysis, ATS compatibility score, resume parsing insights,
                  keyword optimization recommendations, and personalized
                  feedback to increase your interview chances.
                </p>
              </div>
                ...
                <div className="mt-6 rounded-2xl border border-slate-200/80 bg-white/80 p-4 shadow-sm">
                  <div className="mb-3 flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <p className="text-xs font-semibold text-slate-500">
                      {scoredResumes.length} scored report
                      {scoredResumes.length === 1 ? "" : "s"}
                    </p>
                  </div>
            ...
            <aside className="rounded-3xl border border-cyan-300/10 bg-[#0B1220] p-6 text-white shadow-[0_24px_90px_rgba(15,23,42,0.18)]">
              ...
              <div className="mt-4 overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03]">
                ...
                {["Parse", "Score", "Diagnose", "Improve"].map(
                  (step, index) => (
                    <div
                      key={step}
              ...
              filteredResumes.length === 0 && (
                <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-8 py-14 text-center">
                  ...
                  <p className="max-w-md text-sm leading-6 text-slate-500">
                    Adjust the search term or switch back to all reports to
                    restore the full library.
                  </p>
                  <button
                    type="button"
                    onClick={() => {
            ...
            {!loadingResumes && resumes?.length === 0 && (
              <div className="relative overflow-hidden rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-8 py-14 text-center">
                ...
                <div className="relative flex flex-col items-center justify-center gap-4">
                  ...
                  <p className="max-w-md text-sm leading-6 text-slate-500">
                    Upload a PDF resume and paste the job description to
                    generate your first signal report.
                  </p>
                  <Link to="/upload" className="primary-button mt-2 w-fit">
                    Upload Resume
                  </Link>
                  <div className="mt-6 grid w-full max-w-2xl gap-3 sm:grid-cols-3">
                    {["Upload PDF", "Run AI scan", "Improve match"].map(
                      (step) => (
                        <div
                          key={step}
