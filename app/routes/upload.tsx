...
const upload = () => {
  ...
  const handleAnalyse = async ({
    ...
    setStatusText('Converting the data to image ...');
    const imageFile = await convertPdfToImage(file);

    if(!imageFile.file) return setStatusText('Error : failed to convert pdf into image');
    ...
    <main className="bg-[url('/images/bg-main.svg')] bg-cover">
      ...
      <section className="main-section">
        <div className="page-heading py-16">
          <h1>Smart feedback for your dream hob</h1>
          {isProcessing ? (
            <>
              <h2>{statusText}</h2>
